...
def predict(t,s,th):
	dir_json_path="./GS/" ##Tagged
	dir_in_path="./6_Out_pkl/"+t
	dir_out_path="./7_Course/"
	if not os.path.exists(dir_out_path):
		os.makedirs(dir_out_path)
	olist = []
	glist = []
	wdata = []
	olist.extend(run(community_construction_8,dir_in_path,s,th)) ## function name 1/2/4/5/7*/8
	with open(dir_json_path + t +'.json') as json_file:
		...
		for d in data:
			...
			for gg in d['gold']:
				glist.append(gg)
			wdata.append({"id":d['id'],"topics":d['topics'],"gold":d['gold'],"course":offlist})
...
if __name__=="__main__":
	...
	type_sim = [1.0] # [0.0,0.5,1.0]
	threshold = [0.0] # [0.0,0.01,0.02,0.04,0.06,0.08,0.1]
	for s in type_sim:
		for th in threshold:
			print("Sim: " + str(s) + " Thres: " + str(th))
			C = 0
			P = 0
			CP = 0
	...
	print("Course-relevant keys predicted...........")
